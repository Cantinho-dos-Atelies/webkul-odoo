        ...
        if kw.get("name", False):
            if 'error' not in qcontext and request.httprequest.method == 'POST':
                try:
                    ...
                    # if user:
                    #     group = request.env.ref('product.group_product_pricelist')
                    #     group_mp_product_pricelist = request.env['ir.default']._get('res.config.settings', 'group_mp_product_pricelist')
                    #     if group and not group_mp_product_pricelist:
                    #         _logger.info("-----seller_signup_form---3------%r--",[group.users])

                    #         group.sudo().write({"users": [(3, user.id, 0)]})
                    #         _logger.info("-----seller_signup_form--4-------%r--",[group.users])
...
class WKsellerMP(http.Controller):
    ...
    def wk_check_mp_seller(self, **post):
        _logger.info(f'----request.env.user.check_user_is_draft_or_approved_seller()----------{request.env.user.check_user_is_draft_or_approved_seller()}---------------')
